...
class Message(urwid.AttrMap):
    ...
    def __init__(
        self, ts, channel_id, user, text, indicators, reactions=(), attachments=()
    ):
        self.ts = ts
        ...
        if reactions:
            main_column.append(
                urwid.Columns([("pack", reaction) for reaction in reactions])
            )
    ...
    def keypress(self, size, key):
        ...
        if key == keymap["delete_message"]:
            ...
            urwid.emit_signal(
                self, "edit_message", self, self.user_id, self.ts, self.original_text
            )
            return True
            ...
            for item in self.markdown_text.markup:
                ...
                if type == "link" and re.compile(r"^https?://").search(value):
                    browser_instance = (
                        webbrowser
                        if browser_name == ""
                        else webbrowser.get(browser_name)
                    )
