...
def placeholder(size=10, left=0):
    return (" " * left)[:left] + (get_icon("square") * size)[:size]
...
class CircularLoading(urwid.Pile):
    ...
    def __init__(self):
        self._index = 0
        self._shape = [
            urwid.Text([char for char in row], align="center") for row in self._matrix
        ]
    ...
    def next_frame(self):
        ...
        active_text[active_column] = (
            "loading_active_block",
            active_text[active_column],
        )
...
class LoadingSideBar(urwid.Frame):
    def __init__(self):
        ...
        body = urwid.ListBox(
            [urwid.Text(placeholder(size=20, left=2)), divider]
            + [
